...
interface RegularSetting { jrHigh: number; highSchool: number; }
interface WageSettings { admin: number; jrHigh: number; highSchool: number; tutor: number; transport: number; }
...
function loadData() {
    const s = localStorage.getItem(KEY_SHIFT); if (s) shiftDataList = JSON.parse(s);
    const r = localStorage.getItem(KEY_REGULAR); if (r) regularSettings = JSON.parse(r);
    const w = localStorage.getItem(KEY_WAGE); if (w) wageSettings = JSON.parse(w);
}
...
window.onload = () => {
    ...
    // 時給設定のフォームに初期値をセット
    (document.getElementById('wage-admin') as HTMLInputElement).value = wageSettings.admin.toString();
    (document.getElementById('wage-jrhigh') as HTMLInputElement).value = wageSettings.jrHigh.toString();
    (document.getElementById('wage-high') as HTMLInputElement).value = wageSettings.highSchool.toString();
    (document.getElementById('wage-tutor') as HTMLInputElement).value = wageSettings.tutor.toString();
    (document.getElementById('wage-transport') as HTMLInputElement).value = wageSettings.transport.toString();
};
...
function setupDateSelectors() {
    ...
    for (let y = now.getFullYear() - 1; y <= now.getFullYear() + 1; y++) yearSelect.options.add(new Option(y.toString(), y.toString()));
    for (let m = 1; m <= 12; m++) monthSelect.options.add(new Option(m.toString(), m.toString()));
...
function initCalendar() {
    ...
    if (!calendarDiv) return; // 安全のためのチェック
    calendarDiv.innerHTML = '';

    const startDate = new Date(year, month - 1, 16);
    const endDate = new Date(year, month, 15);
    
    for (let i = 0; i < startDate.getDay(); i++) calendarDiv.appendChild(document.createElement('div'));
    ...
    while (currentDate <= endDate) {
        ...
        if (shift.jrHigh > 0) html += `<div class="koma-row"><span class="material-symbols-outlined icon-jrhigh">backpack</span> ${shift.jrHigh}</div>`;
        if (shift.highSchool > 0) html += `<div class="koma-row"><span class="material-symbols-outlined icon-hs">school</span> ${shift.highSchool}</div>`;
        if (shift.tutor > 0) html += `<div class="koma-row"><span class="material-symbols-outlined icon-tutor">local_library</span> ${shift.tutor}</div>`;
...
function selectDate(dateStr: string, dayOfWeek: number) {
...
function updateKoma(type: 'jrHigh' | 'highSchool', amount: number) {
    const shift = getShift(selectedDateStr);
    if (type === 'jrHigh') shift.jrHigh = Math.max(0, shift.jrHigh + amount);
    else shift.highSchool = Math.max(0, shift.highSchool + amount);
    saveShifts(); updatePanelDisplay(); initCalendar();
}
...
function updateModalTutor(amount: number) {
    const shift = getShift(selectedDateStr);
    shift.tutor = Math.max(0, shift.tutor + amount);
    document.getElementById('modal-tutor-count')!.innerText = shift.tutor.toString();
    saveShifts();
}
function updateModalAdjust(amount: number) {
    const shift = getShift(selectedDateStr);
    shift.adjust += amount;
    document.getElementById('modal-adjust-count')!.innerText = `${shift.adjust}分`;
    saveShifts();
...
function renderRegularSettings() {
    const container = document.getElementById('regular-container') as HTMLDivElement;
    container.innerHTML = '';
...
function changeReg(day: number, type: 'jrHigh' | 'hs', amount: number) {
    if (type === 'jrHigh') regularSettings[day].jrHigh = Math.max(0, regularSettings[day].jrHigh + amount);
    else regularSettings[day].highSchool = Math.max(0, regularSettings[day].highSchool + amount);
    renderRegularSettings();
    saveRegularSettings();
}
function applyRegularShifts() {
    const year = parseInt((document.getElementById('select-year') as HTMLSelectElement).value);
    const month = parseInt((document.getElementById('select-month') as HTMLSelectElement).value);
    const startDate = new Date(year, month - 1, 16);
    const endDate = new Date(year, month, 15);
    ...
    initCalendar();
    const shiftTabBtn = document.querySelectorAll('.tab-btn')[0] as HTMLElement;
...
(window as any).updateModalTutor = updateModalTutor;
(window as any).updateModalAdjust = updateModalAdjust;
(window as any).changeReg = changeReg;
(window as any).saveRegularSettings = saveRegularSettings;
(window as any).applyRegularShifts = applyRegularShifts;
(window as any).saveWageSettings = saveWageSettings;
